...
export const createPatient = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    !["identificado", "não identificado"].includes(req.body.patientType)
  ) {
    const error = new Error(
      "Tipo de paciente inválido. Deve ser 'Identificado' ou 'não identificado'"
    );
  ...
  if (req.body.patientType === "identificado") {
    if (!req.body.name) {
      const error = new Error(
        "Nome é obrigatório para pacientes identificados"
      );
      error.statusCode = 400;
      return next(error);
    }
    ...
    if (req.body.cpf) {
      ...
      if (existingPatient) {
        const error = new Error(
          `Já existe um paciente com o CPF ${req.body.cpf}`
        );
        error.statusCode = 400;
        return next(error);
      }
    ...
    if (!req.body.referenceCode) {
      const error = new Error(
        "Código de referência é obrigatório para pacientes não identificados"
      );
      error.statusCode = 400;
      return next(error);
    }
    ...
    if (existingPatient) {
      const error = new Error(
        `Já existe um paciente com o código de referência ${req.body.referenceCode}`
      );
      error.statusCode = 400;
      return next(error);
    }
  ...
  if (req.body.caseId) {
    ...
    if (
      ...
      const error = new Error(
        `Usuário ${req.user.id} não está autorizado a vincular pacientes a este caso`
      );
...
export const updatePatient = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    req.body.referenceCode !== patient.referenceCode
  ) {
    const existingPatient = await Patient.findOne({
      referenceCode: req.body.referenceCode,
    });
...
export const addCaseToPatient = asyncHandler(async (req, res, next) => {
  ...
  // Todos os usuários autorizados podem vincular pacientes a qualquer caso

  // Adicionar caso ao paciente
  patient.addCase(
    req.body.caseId,
    req.body.relationshipType || "outro",
    req.body.notes || ""
  );
...
export const removeCaseFromPatient = asyncHandler(async (req, res, next) => {
  ...
  // Remover a verificação de propriedade do caso
  // Todos os usuários autorizados podem desvincular pacientes de qualquer caso

  // Verificar se o caso está vinculado ao paciente
  const caseIndex = patient.cases.findIndex(
    (c) => c.caseId.toString() === req.params.caseId
  );

  if (caseIndex === -1) {
    const error = new Error(
      `Caso ${req.params.caseId} não está vinculado ao paciente ${req.params.id}`
    );
    error.statusCode = 404;
    return next(error);
  }
...
export const addDentalFeature = asyncHandler(async (req, res, next) => {
  ...
  if (!req.body.toothNumber || !req.body.description || !req.body.type) {
    const error = new Error(
      `Dados incompletos. Forneça toothNumber, description e type`
    );
    error.statusCode = 400;
    return next(error);
  }
...
export const searchByDentalFeatures = asyncHandler(async (req, res, next) => {
  ...
  const patients = await Patient.find(query)
    ...
    .limit(limit)
    .select(
      "patientType name referenceCode gender estimatedGender status dentalFeatures"
    )
  ...
  const patientsWithScore = patients.map((patient) => {
    ...
    features.forEach((searchFeature) => {
      patientFeatures.forEach((patientFeature) => {
        if (
          patientFeature.toothNumber === searchFeature.toothNumber &&
          patientFeature.type === searchFeature.type
        ) {
          score += 1;
        }
      });
...
export const getCasePatients = asyncHandler(async (req, res, next) => {
  ...
  // Remover a verificação de propriedade do caso
  // Todos os usuários podem acessar pacientes de qualquer caso

  const patients = await Patient.find({ "cases.caseId": req.params.caseId })
    .select(
      "patientType name referenceCode gender estimatedGender status cases"
    )
    .sort("patientType name referenceCode");
