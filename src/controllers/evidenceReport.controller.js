...
export const getEvidenceReport = asyncHandler(async (req, res, next) => {
  ...
  // Todos os usuários podem acessar qualquer relatório de evidência
...
export const createEvidenceReport = asyncHandler(async (req, res, next) => {
  ...
  if (!evidence) {
    return next(
      new ErrorResponse(
        `Evidência não encontrada com id ${req.body.evidence}`,
        404
      )
    );
  }
  ...
  if (!forensicCase) {
    return next(
      new ErrorResponse(`Caso não encontrado com id ${req.body.case}`, 404)
    );
  }
...
export const updateEvidenceReport = asyncHandler(async (req, res, next) => {
  ...
  // Remover a verificação de propriedade do caso
  // Todos os usuários autorizados podem atualizar qualquer relatório de evidência
  ...
  if (evidenceReport.status === "assinado") {
    return next(
      new ErrorResponse(
        "Não é possível modificar um relatório já assinado",
        400
      )
    );
  }

  evidenceReport = await EvidenceReport.findByIdAndUpdate(
    req.params.id,
    req.body,
    {
      new: true,
...
export const deleteEvidenceReport = asyncHandler(async (req, res, next) => {
  ...
  // Remover a verificação de propriedade do caso
  // Todos os usuários autorizados podem excluir qualquer relatório de evidência

  // Não permitir exclusão de relatórios assinados
  if (evidenceReport.status === "assinado") {
    return next(
      new ErrorResponse(`Não é possível excluir um relatório assinado`, 400)
    );
  }
...
// @access  Privado
export const getEvidenceReportsByEvidence = asyncHandler(
  async (req, res, next) => {
    ...
    // Remover a verificação de propriedade do caso
    // Todos os usuários podem acessar relatórios para qualquer evidência

    const evidenceReports = await EvidenceReport.find({
      evidence: req.params.evidenceId,
    }).populate("expertResponsible", "name email");
...
export const signEvidenceReport = asyncHandler(async (req, res, next) => {
  ...
  // Remover a verificação de propriedade do caso
  // Todos os usuários autorizados podem assinar qualquer relatório de evidência
  ...
  if (evidenceReport.status !== "finalizado") {
    return next(
      new ErrorResponse(
        `Relatório deve estar no status 'finalizado' para ser assinado`,
        400
      )
    );
  }
...
// @access  Privado
export const verifyEvidenceReportSignature = asyncHandler(
  async (req, res, next) => {
    const evidenceReport = await EvidenceReport.findById(
      req.params.id
    ).populate("expertResponsible", "name email");
    ...
    if (
      evidenceReport.status !== "assinado" ||
      !evidenceReport.digitalSignature
    ) {
...
// @access  Público
export const verifyEvidenceReportByHash = asyncHandler(
  async (req, res, next) => {
    const { hash, code } = req.query;

    if (!hash || !code) {
      return next(
        new ErrorResponse(`Hash e código de verificação são obrigatórios`, 400)
      );
    }
    ...
    const verificationResult = verifyDocumentByHash(
      req.params.id,
      hash,
      code,
      evidenceReport
